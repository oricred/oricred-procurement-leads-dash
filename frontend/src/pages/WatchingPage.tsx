  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-6">
        ...
        <h2 className="text-lg font-semibold text-white">Watching Board</h2>
        <span className="text-xs px-2 py-0.5 rounded-full bg-surface-300 text-gray-400">
          {data?.total ?? 0} tenders
        </span>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="glass rounded-xl p-12 text-center">
          ...
          <p className="text-sm text-gray-600 mt-1">New qualified tenders will appear here automatically.</p>
        </div>
      ) : (
            ...
            return (
              <div key={item.id} className="glass rounded-xl p-4 card-hover">
                ...
                <div className="flex items-start justify-between mb-3">
                  ...
                  <StatusIcon className={`w-4 h-4 flex-shrink-0 ${statusColors[item.status] ?? 'text-gray-400'}`} />
                </div>
